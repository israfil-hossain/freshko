            ...
            categories.map((cat) => {
              ...
              return (
                <div key={cat._id}>
                  ...
                  >
                    {cat.image && (
                      <img src={cat.image} alt="" className="w-5 h-5 object-cover rounded flex-shrink-0" />
                    )}
                    <span className="flex-1 text-left truncate">{cat.name}</span>
                    {hasSubs && (
                      <svg
                        className={`w-4 h-4 flex-shrink-0 transition-transform ${active ? "text-white/70" : "text-gray-300"} ${expanded ? "rotate-90" : ""}`}
